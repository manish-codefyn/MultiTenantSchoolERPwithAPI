...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--tenant", type=str, help="Schema name of the tenant", default="public"
        )
    ...
    def handle(self, *args, **options):
        ...
        with tenant_context(tenant):
            ...
            self.stdout.write(
                f"Loading dummy data from: {file_path} for tenant {tenant.schema_name}"
            )
        ...
        try:
            ...
            # Create users first (required for staff and HODs)
            ...
            staff_members = self.create_staff(
                data.get("staff", []), users, departments, designations
            )
            ...
            self.create_leave_applications(
                data.get("leave_applications", []), staff_members, leave_types
            )

            # Create leave balances
            self.create_leave_balances(
                data.get("leave_balances", []), staff_members, leave_types
            )

            # Create salary structures
            self.create_salary_structures(
                data.get("salary_structures", []), staff_members
            )
            ...
            self.create_promotions(
                data.get("promotions", []), staff_members, designations
            )

            # Create employment history
            self.create_employment_history(
                data.get("employment_history", []), staff_members
            )

            # Create training programs
            training_programs = self.create_training_programs(
            ...
            self.create_performance_reviews(
                data.get("performance_reviews", []), staff_members
            )

            # Create recruitments
            recruitments = self.create_recruitments(
                data.get("recruitments", []), departments, designations
            )
    ...
    def create_departments(self, departments_data):
        ...
        for dept_data in departments_data:
            try:
                ...
                self.stderr.write(
                    f"Error creating department {dept_data.get('name', 'Unknown')}: {str(e)}"
                )
        return departments
        ...
        for desig_data in designations_data:
            try:
                ...
                designation, created = Designation.objects.get_or_create(
                    code=desig_data["code"], defaults=desig_data
                )

                if created and reports_to_data:
                    reports_to = Designation.objects.filter(
                        code=reports_to_data
                    ).first()
                    if reports_to:
                ...
                self.stderr.write(
                    f"Error creating designation {desig_data.get('title', 'Unknown')}: {str(e)}"
                )
        return designations
    ...
    def create_users(self, users_data):
        ...
        for user_data in users_data:
            try:
                ...
                self.stderr.write(
                    f"Error creating user {user_data.get('username', 'Unknown')}: {str(e)}"
                )
        return users
    ...
    def create_staff(self, staff_data, users, departments, designations):
        ...
        for staff_info in staff_data:
            try:
                ...
                if not user:
                    self.stderr.write(
                        f"User {username} not found for staff {employee_id}"
                    )
                    continue
                ...
                if not department or not designation:
                    self.stderr.write(
                        f"Department or designation not found for staff {employee_id}"
                    )
                    continue
                ...
                for field in date_fields:
                    if field in staff_info and staff_info[field]:
                        staff_info[field] = datetime.strptime(
                            staff_info[field], "%Y-%m-%d"
                        ).date()
                ...
                if created:
                    ...
                    self.stdout.write(
                        f"Created staff: {staff.employee_id} - {staff.full_name}"
                    )
            except Exception as e:
                self.stderr.write(
                    f"Error creating staff {staff_info.get('employee_id', 'Unknown')}: {str(e)}"
                )
            ...
            try:
                ...
                for field in date_fields:
                    if field in doc_data and doc_data[field]:
                        doc_data[field] = datetime.strptime(
                            doc_data[field], "%Y-%m-%d"
                        ).date()
    ...
    def create_attendance(self, attendance_data, staff_members):
        ...
        for att_data in attendance_data:
            try:
                ...
                # Convert date
                att_data["date"] = datetime.strptime(
                    att_data["date"], "%Y-%m-%d"
                ).date()
                ...
                for field in time_fields:
                    if field in att_data and att_data[field]:
                        att_data[field] = datetime.strptime(
                            att_data[field], "%H:%M:%S"
                        ).time()
                ...
                StaffAttendance.objects.get_or_create(
                    staff=staff,
                    date=att_data["date"],
                    defaults={"marked_by": marked_by, **att_data},
                )
            except Exception as e:
        ...
        for lt_data in leave_types_data:
            try:
                ...
                if approval_auth_code:
                    approval_auth = Designation.objects.filter(
                        code=approval_auth_code
                    ).first()
        ...
        for app_data in applications_data:
            try:
                ...
                if not staff or not leave_type:
                    self.stderr.write(
                        f"Staff or leave type not found for leave application"
                    )
                    continue

                # Convert dates
                app_data["start_date"] = datetime.strptime(
                    app_data["start_date"], "%Y-%m-%d"
                ).date()
                app_data["end_date"] = datetime.strptime(
                    app_data["end_date"], "%Y-%m-%d"
                ).date()
                ...
                self.stdout.write(
                    f"Created leave application for staff: {staff.employee_id}"
                )
            except Exception as e:
    ...
    def create_leave_balances(self, balances_data, staff_members, leave_types):
        ...
        for balance_data in balances_data:
            try:
                ...
                if not staff or not leave_type:
                    self.stderr.write(
                        f"Staff or leave type not found for leave balance"
                    )
                    continue
    ...
    def create_salary_structures(self, structures_data, staff_members):
        ...
        for struct_data in structures_data:
            try:
                ...
                if not staff:
                    self.stderr.write(
                        f"Staff {employee_id} not found for salary structure"
                    )
                    continue
                ...
                self.stdout.write(
                    f"Created salary structure for staff: {staff.employee_id}"
                )
            except Exception as e:
    ...
    def create_employment_history(self, history_data, staff_members):
        ...
        for hist_data in history_data:
            try:
                ...
                if not staff:
                    self.stderr.write(
                        f"Staff {employee_id} not found for employment history"
                    )
                    continue
                ...
                if initiated_by_username:
                    initiated_by = User.objects.filter(
                        username=initiated_by_username
                    ).first()
                if approved_by_username:
                    approved_by = User.objects.filter(
                        username=approved_by_username
    ...
    def create_training_programs(self, training_data):
        ...
        for prog_data in training_data:
            try:
                code = (
                    prog_data.get("code")
                    or prog_data["title"].replace(" ", "_").upper()
                )
    ...
    def create_training_participations(
        self, participations_data, staff_members, training_programs
    ):
        ...
        for part_data in participations_data:
            try:
                ...
                if not staff or not training:
                    self.stderr.write(
                        f"Staff or training program not found for participation"
                    )
                    continue
                ...
                TrainingParticipation.objects.get_or_create(
                    training=training, staff=staff, defaults=part_data
                )
                self.stdout.write(
                    f"Created training participation for staff: {staff.employee_id}"
                )
            except Exception as e:
    ...
    def create_performance_reviews(self, reviews_data, staff_members):
        ...
        for review_data in reviews_data:
            try:
                ...
                if not staff:
                    self.stderr.write(
                        f"Staff {employee_id} not found for performance review"
                    )
                    continue
                ...
                if reviewed_by_username:
                    reviewed_by = User.objects.filter(
                ...
                self.stdout.write(
                    f"Created performance review for staff: {staff.employee_id}"
                )
            except Exception as e:
    ...
    def create_recruitments(self, recruitments_data, departments, designations):
        ...
        for rec_data in recruitments_data:
            try:
                ...
                if not department or not designation:
                    self.stderr.write(
                        f"Department or designation not found for recruitment"
                    )
                    continue
                ...
                if hiring_manager_username:
                    hiring_manager = User.objects.filter(
                        username=hiring_manager_username
                    ).first()
                ...
                recruitment, created = Recruitment.objects.get_or_create(
                    ...
                    department=department,
                    designation=designation,
                    defaults={"hiring_manager": hiring_manager, **rec_data},
                )

                if created:
                    recruitments[rec_code] = recruitment
                    self.stdout.write(
                        f"Created recruitment: {recruitment.position_title}"
                    )
            except Exception as e:
    ...
    def create_job_applications(self, applications_data, recruitments):
        ...
        for app_data in applications_data:
            try:
                ...
                if not recruitment:
                    self.stderr.write(
                        f"Recruitment {rec_code} not found for job application"
                    )
                    continue
                ...
                self.stdout.write(
                    f"Created job application: {app_data['applicant_name']}"
                )
            except Exception as e:
