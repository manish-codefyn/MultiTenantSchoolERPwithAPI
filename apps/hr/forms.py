...
from .models import (
    ...
    DisciplinaryAction, Asset, AssetAssignment
)
...
class StaffBasicForm(TenantAwareModelForm):
...
class StaffEmergencyContactForm(TenantAwareModelForm):
    class Meta:
        model = Staff
        fields = [
...
class StaffAcademicForm(TenantAwareModelForm):
    class Meta:
        model = Staff
        fields = [
...
class StaffBankDetailsForm(TenantAwareModelForm):
    class Meta:
        model = Staff
        fields = [
            'bank_account_number', 'bank_name', 'ifsc_code', 'basic_salary'
        ]

class StaffIdentificationForm(TenantAwareModelForm):
    class Meta:
        model = Staff
        fields = [
...
class TrainingProgramForm(TenantAwareModelForm):
    class Meta:
        ...
        fields = [
            'title', 'description', 'training_type', 'provider',
            'duration_hours', 'location', 'cost', 'maximum_participants',
...
class DisciplinaryActionForm(TenantAwareModelForm):
    class Meta:
        model = DisciplinaryAction
        fields = [
            'staff', 'action_type', 'incident_date', 'action_date',
            'description', 'violation', 'action_details',
            'duration_days', 'fine_amount', 'issued_by',
            'reviewed_by_hr', 'follow_up_date', 'follow_up_notes'
        ]
        widgets = {
            ...
            'description': forms.Textarea(attrs={'rows': 3}),
            'violation': forms.Textarea(attrs={'rows': 3}),
            'action_details': forms.Textarea(attrs={'rows': 3}),
            'follow_up_notes': forms.Textarea(attrs={'rows': 3}),
        }

class AssetForm(TenantAwareModelForm):
    class Meta:
        model = Asset
        fields = [
            'asset_id', 'name', 'asset_type', 'brand', 'model',
            'serial_number', 'purchase_date', 'purchase_cost',
            'warranty_expiry', 'current_status', 'location', 'notes'
        ]
        widgets = {
...
class AssetAssignmentForm(TenantAwareModelForm):
    class Meta:
        model = AssetAssignment
        fields = [
            'staff', 'asset', 'assigned_date', 'expected_return_date',
            'condition_at_assignment', 'purpose', 'assigned_by', 'notes', 'is_active'
        ]
        widgets = {
            'assigned_date': forms.DateInput(attrs={'type': 'date'}),
            'expected_return_date': forms.DateInput(attrs={'type': 'date'}),
            'condition_at_assignment': forms.Textarea(attrs={'rows': 3}),
