...
class Staff(BaseModel):
    ...
    def create_user_account(self):
        """Create a user account for the staff member if one doesn't exist"""
        if self.user:
            return self.user
        ...
        # Determine username (use email or employee_id)
        username = self.personal_email
        email = self.personal_email
        
        # Determine role based on designation or default to 'staff'
        role = 'staff'
        # Basic logic: if title contains 'Teacher' or 'Faculty', set as teacher
        if self.designation and ('Teacher' in self.designation.title or 'Faculty' in self.designation.title):
            role = 'teacher'
        ...
        try:
            ...
            if User.objects.filter(email=email).exists():
                ...
                user = User.objects.create_user(
                    username=username,
                    email=email,
                    password=None,  # Set unusable password initially
            ...
            print(f"Error creating user for staff {self.pk}: {e}")
            return None
...
class TrainingProgram(BaseModel):
    ...
    provider = models.CharField(max_length=200, verbose_name=_("Training Provider"))
    ...
    def enrolled_count(self):
        return self.training_participations.filter(status="ENROLLED").count()

    @property
    def remaining_seats(self):
        if self.maximum_participants:
            return self.maximum_participants - self.enrolled_count
        return None
...
class PerformanceReview(BaseModel):
    ...
    review_date = models.DateField(default=timezone.localdate, verbose_name=_("Review Date"))
...
class Recruitment(BaseModel):
    ...
    # Timeline
    posting_date = models.DateField(default=timezone.localdate, verbose_name=_("Posting Date"))
...
class DisciplinaryAction(BaseModel):
    ...
    description = models.TextField(verbose_name=_("Incident Description"))
    violation = models.TextField(verbose_name=_("Policy Violation"))
    
    # Action Details
    action_details = models.TextField(verbose_name=_("Action Details"))
    ...
    reviewed_by_hr = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="reviewed_disciplinary_actions",
        verbose_name=_("Reviewed by HR")
    )
    acknowledged_by_staff = models.BooleanField(
        default=False,
        verbose_name=_("Acknowledged by Staff")
    )
    staff_comments = models.TextField(
        blank=True,
        verbose_name=_("Staff Comments")
    )
    ...
    def __str__(self):
        return f"{self.staff} - {self.get_action_type_display()} ({self.action_date})"

class Asset(BaseModel):
    """
    Company assets assigned to staff
    """
    ASSET_TYPE_CHOICES = (
        ("LAPTOP", "Laptop"),
...
class AssetAssignment(BaseModel):
    ...
    assigned_date = models.DateField(verbose_name=_("Assigned Date"))
    ...
    purpose = models.TextField(blank=True, verbose_name=_("Purpose of Assignment"))
    ...
    def __str__(self):
        return f"{self.asset} assigned to {self.staff}"

    def save(self, *args, **kwargs):
        if self.is_active and not self.actual_return_date:
            # Update asset status to ASSIGNED
            self.asset.current_status = "ASSIGNED"
            self.asset.location = f"Assigned to: {self.staff}"
            self.asset.save()
        elif not self.is_active and self.actual_return_date:
            # Update asset status to AVAILABLE
            self.asset.current_status = "AVAILABLE"
            self.asset.location = "Store/Inventory"
            self.asset.save()
        super().save(*args, **kwargs)