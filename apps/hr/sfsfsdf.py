...
from .forms import (
    ...
    PublicJobApplicationForm, StaffImportForm, QualificationForm,
    HolidayForm, WorkScheduleForm, StaffBasicForm, StaffEmergencyContactForm,
    StaffAcademicForm, StaffBankDetailsForm, StaffIdentificationForm,
    DisciplinaryActionForm, AssetForm, AssetAssignmentForm
)
...
class HRDashboardView(BaseTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["teaching_staff"] = staff_queryset.filter(
            designation__category="TEACHING"
        ).count()
        context["non_teaching_staff"] = staff_queryset.filter(
            designation__category="NON_TEACHING"
        ).count()
        context["administrative_staff"] = staff_queryset.filter(
            designation__category="ADMINISTRATIVE"
        ).count()

        # Department Statistics
        departments = Department.objects.filter(tenant=tenant).annotate(
            staff_members_count=Count(
                "staff_members", filter=Q(staff_members__is_active=True)
            )
        )
        ...
        context["present_today"] = StaffAttendance.objects.filter(
            tenant=tenant, date=today, status__in=["PRESENT", "LATE"]
        ).count()
        ...
        context["on_leave_today"] = StaffAttendance.objects.filter(
            tenant=tenant, date=today, status="LEAVE"
        ).count()
        ...
        context["recent_leaves"] = (
            LeaveApplication.objects.filter(tenant=tenant)
        ...
        recent_reviews = PerformanceReview.objects.filter(
            tenant=tenant, review_date__gte=timezone.now() - timezone.timedelta(days=90)
        )
...
class TeacherListView(BaseListView):
    model = Staff
    template_name = "hr/staff/teacher_list.html"
    context_object_name = "teachers"
    permission_required = "hr.view_staff"

    def get_queryset(self):
        tenant = get_current_tenant()
        # Ensure we're inheriting the correct base logic or calling super appropriately
        # If BaseListView.get_queryset does existing filtering, we chain it
        queryset = (
            super()
            .get_queryset()
            .filter(tenant=tenant, is_active=True, user__role="teacher")
            .select_related("user", "department", "designation")
            .prefetch_related("addresses")
        )
...
class StaffDetailView(BaseDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["leave_applications"] = staff.leave_applications.all().order_by(
            "-start_date"
        )[:10]
        ...
        context["performance_reviews"] = staff.performance_reviews.all().order_by(
            "-review_date"
        )

        # Leave balances
        context["leave_balances"] = LeaveBalance.objects.filter(
            staff=staff, year=timezone.now().year
        ).select_related("leave_type")

        # Recent payroll
        context["recent_payroll"] = Payroll.objects.filter(staff=staff).order_by(
            "-salary_month"
        )[:6]

        # Training participation
        context["trainings"] = (
            TrainingParticipation.objects.filter(staff=staff)
...
class StaffProfilePrintView(BaseDetailView):
    """
    Render a clean, printable resume-style profile page.
    """

    model = Staff
    template_name = "hr/staff/profile_print.html"
    permission_required = "hr.view_staff"
    context_object_name = "staff"
    ...
    def get_context_data(self, **kwargs):
        ...
        # Add all related data needed for a full resume
        context["addresses"] = staff.addresses.all()
        context["qualifications"] = staff.qualifications.all().order_by("-year")
        context["experience"] = staff.employment_history.all().order_by(
            "-start_date"
        )  # Assuming logic exists or using similar
        return context


class StaffOnboardingView(
    LoginRequiredMixin, TenantAccessMixin, RoleRequiredMixin, DetailView
):
    """
    Central hub for staff registration progress.
    Displays completion status of all steps and links to edit/add them.
    """

    model = Staff
    template_name = "hr/staff/onboarding.html"
    context_object_name = "staff"
    roles_required = ["teacher", "staff", "hr_manager", "admin"]

    def get_object(self, queryset=None):
        # Allow viewing own profile or if HR view specific staff
        if "pk" in self.kwargs:
            obj = super().get_object(queryset)
            # Security check
            if obj.user != self.request.user and not self.request.user.role in [
                "hr",
                "admin",
                "hr_manager",
            ]:
            ...
            # Default to current user's staff profile
    ...
    def get_context_data(self, **kwargs):
        ...
        steps = [
            {
                ...
                "description": _("Personal and employment details"),
                "is_completed": True,
                "url": reverse_lazy("hr:staff_update_basic", kwargs={"pk": staff.pk}),
                "icon": "bx bx-user",
                "step_number": 1,
            },
            {
                ...
                "description": _("Current and permanent address"),
                "is_completed": staff.addresses.exists(),
                "url": reverse_lazy(
                    "hr:staff_address_create", kwargs={"staff_id": staff.pk}
                ),
                "icon": "bx bx-map-pin",
                "step_number": 2,
            },
            {
                ...
                "description": _("Login credentials setup"),
                "is_completed": staff.user is not None and staff.user.has_usable_password(),
                "url": reverse_lazy("hr:staff_account_info", kwargs={"staff_id": staff.pk}),
                "icon": "bx bx-lock-alt",
                "step_number": 3,
            },
            {
                ...
                "description": _("Emergency contact information"),
                "is_completed": bool(staff.emergency_contact_phone),
                "url": reverse_lazy("hr:staff_emergency_update", kwargs={"pk": staff.pk}),
            ...
            },
            {
                ...
                "description": _("Government IDs and documents"),
                "is_completed": bool(staff.pan_number or staff.aadhaar_number),
                "url": reverse_lazy("hr:staff_identification_update", kwargs={"pk": staff.pk}),
                "icon": "bx bx-id-card",
                "step_number": 5,
                "optional": True,
            },
            {
                "id": "academic",
                "title": _("Academic & Experience"),
                "description": _("Qualifications and work history"),
                "is_completed": bool(staff.qualifications.exists() or staff.specialization),
                "url": reverse_lazy("hr:staff_academic_update", kwargs={"pk": staff.pk}),
            ...
            },
            {
                ...
                "description": _("Salary account information"),
                "is_completed": bool(staff.bank_account_number),
                "url": reverse_lazy("hr:staff_bank_update", kwargs={"pk": staff.pk}),
            ...
            },
            {
                ...
                "description": _("Required certificates and documents"),
                "is_completed": staff.documents.exists(),
                "url": reverse_lazy(
                    "hr:staff_document_upload", kwargs={"staff_id": staff.pk}
                ),
                "icon": "bx bx-file",
                "step_number": 8,
            },
        ...
        completed_required = sum(1 for step in required_steps if step["is_completed"])
        total_required = len(required_steps)
        progress = (
            (completed_required / total_required) * 100 if total_required > 0 else 100
        )
...
class StaffCreateView(BaseCreateView):
    model = Staff
    form_class = StaffBasicForm
    template_name = "hr/staff/onboarding_step_form.html"
    ...
    def get_success_url(self):
        return reverse_lazy("hr:staff_onboarding", kwargs={"pk": self.object.pk})
...
class StaffBasicUpdateView(BaseUpdateView):
    """Step 1: Edit Basic Staff Information"""
    model = Staff
    form_class = StaffBasicForm
    template_name = "hr/staff/onboarding_step_form.html"
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = _("Basic Information")
        return context
...
class StaffEmergencyUpdateView(BaseUpdateView):
    """Step 2: Update Emergency Contact"""
    model = Staff
    form_class = StaffEmergencyContactForm
    template_name = "hr/staff/onboarding_step_form.html"
    permission_required = "hr.change_staff"
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['staff'] = self.object
        context['title'] = _("Emergency Contacts")
        return context

    def get_success_url(self):
        messages.success(self.request, _("Emergency contacts updated."))
        return reverse_lazy("hr:staff_onboarding", kwargs={"pk": self.object.pk})

class StaffAcademicUpdateView(BaseUpdateView):
    """Step 4: Update Academic & Experience"""
    model = Staff
    form_class = StaffAcademicForm
    template_name = "hr/staff/onboarding_step_form.html"
    permission_required = "hr.change_staff"
    ...
    def get_success_url(self):
        messages.success(self.request, _("Academic information updated."))
        return reverse_lazy("hr:staff_onboarding", kwargs={"pk": self.object.pk})

class StaffBankUpdateView(BaseUpdateView):
    """Step 5: Update Bank Details"""
    model = Staff
    form_class = StaffBankDetailsForm
    template_name = "hr/staff/onboarding_step_form.html"
    permission_required = "hr.change_staff"
    ...
    def get_success_url(self):
        messages.success(self.request, _("Bank details updated."))
        return reverse_lazy("hr:staff_onboarding", kwargs={"pk": self.object.pk})

class StaffIdentificationUpdateView(BaseUpdateView):
    """Step 6: Update Identification"""
    model = Staff
    form_class = StaffIdentificationForm
    template_name = "hr/staff/onboarding_step_form.html"
    permission_required = "hr.change_staff"
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['staff'] = self.object
        context['title'] = _("Identification Details")
        return context
...
class StaffAddressCreateView(BaseCreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['staff'] = get_object_or_404(Staff, pk=staff_id)
        context['title'] = _("Add Address")
        return context

    def form_valid(self, form):
        staff_id = self.kwargs.get('staff_id')
        form.instance.staff = get_object_or_404(Staff, pk=staff_id)
        return super().form_valid(form)

    def get_success_url(self):
        messages.success(self.request, _("Address added successfully."))
        return reverse_lazy("hr:staff_onboarding", kwargs={"pk": self.object.staff.pk})
...
class StaffAddressUpdateView(BaseUpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = _("Update Address")
        return context
...
class StaffAccountInfoView(BaseView):
    """Step 8: Account Info (Mirroring Student)"""
    template_name = "hr/staff/account_info.html"
    permission_required = "hr.view_staff"
    ...
    def get(self, request, staff_id):
        ...
        if not staff.user:
            # Auto-create user if missing, similar to student flow
            staff.create_user_account()
            if not staff.user:
                 messages.warning(request, "Could not create user account. Please check email/details.")
                 form = SetPasswordForm(user=None)
            else:
                 messages.info(request, "User account created automatically.")
                 form = SetPasswordForm(user=staff.user)
        else:
            form = SetPasswordForm(user=staff.user)
    ...
    def post(self, request, staff_id):
        ...
        if not staff.user:
             staff.create_user_account()
             if not staff.user:
                messages.error(request, "User account missing and could not be created.")
                return redirect('hr:staff_onboarding', pk=staff.pk)

        form = SetPasswordForm(user=staff.user, data=request.POST)
        
        if form.is_valid():
            form.save()
            # update_session_auth_hash(request, form.user) # Don't log out the admin/HR user!
            messages.success(request, f"Password for {staff.full_name} has been updated.")
            return redirect('hr:staff_onboarding', pk=staff.pk)
...
class StaffDeleteView(BaseDeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        messages.success(request, f"Staff member {staff.full_name} has been deleted.")
...
class StaffIDCardView(BaseDetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not (
            request.user.role in ["admin", "staff", "super_admin", "hr"]
            or staff.user == request.user
        ):
            raise PermissionDenied(
                "You do not have permission to view ID card for this staff member."
            )
        ...
        try:
            ...
            for index, row in enumerate(data_list):
                ...
                if not email or not first_name:
                    errors.append(
                        f"Row {row_num}: Missing required keys. Found: {list(row.keys())}"
                    )
                    continue
                ...
                if not dept_id and row.get("department"):
                    dept_obj = Department.objects.filter(
                        tenant=tenant, name__iexact=row["department"].strip()
                    ).first()
                    if dept_obj:
                        dept_id = dept_obj.id
                ...
                if not desig_id and row.get("designation"):
                    desig_obj = Designation.objects.filter(
                        tenant=tenant, title__iexact=row["designation"].strip()
                    ).first()
                    if desig_obj:
            ...
            if errors:
                messages.warning(
                    request,
                    f"Encounters {len(errors)} errors: " + "; ".join(errors[:5]),
                )
...
class StaffImportSampleView(BaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response["Content-Disposition"] = (
            'attachment; filename="staff_import_sample.csv"'
        )
...
class StaffExportView(ExportMixin, BaseListView):
    ...
    def get_queryset(self):
        return (
            super().get_queryset().select_related("user", "department", "designation")
        )
...
class DepartmentDeleteView(BaseDeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if department.staff_members.filter(is_active=True).exists():
            ...
            return JsonResponse(
                {"success": False, "error": "Department has active staff"}
            )
    ...
    def post(self, request, *args, **kwargs):
        if not request.user.has_perm("hr.add_attendance"):
            return HttpResponseForbidden(
                "You don't have permission to mark StaffAttendance"
            )
        ...
        for staff in Staff.objects.filter(tenant=tenant, is_active=True):
            ...
            if status:
                ...
                if check_in_str:
                    try:
                        check_in_time = timezone.datetime.strptime(
                            check_in_str, "%H:%M"
                        ).time()
                    except ValueError:
                        pass
                ...
                if check_out_str:
                    try:
                        check_out_time = timezone.datetime.strptime(
                            check_out_str, "%H:%M"
                        ).time()
                    except ValueError:
                        pass
        ...
        audit_log(
            ...
            details={"date": date, "created": created_count, "updated": updated_count},
            severity="INFO",
        )
...
class LeaveApplicationListView(BaseListView):
    ...
    def get_queryset(self):
        ...
        if not (
            user.has_perm("hr.view_all_leaves") or user.role in ["admin", "hr_manager"]
        ):
    ...
    def get_queryset(self):
        ...
        if month:
            try:
                ...
                queryset = queryset.filter(
                    salary_month__year=month_date.year,
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.object_list:
            context["total_amount"] = (
                self.object_list.aggregate(total=Sum("net_salary"))["total"] or 0
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        for staff in Staff.objects.filter(
            ...
            if Payroll.objects.filter(
                tenant=tenant, staff=staff, salary_month=month_date
            ).exists():
                continue
        ...
        messages.success(
            request, f"Payroll generated for {created_count} staff members for {month}."
        )
...
class StaffAutocompleteView(BaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        staff_list = (
            Staff.objects.filter(tenant=tenant, is_active=True)
            .filter(filters)
...
class StaffExportView(BaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response = HttpResponse(content_type="text/csv")
        response["Content-Disposition"] = (
            'attachment; filename="staff_list_{}.csv"'.format(
        ...
        # Write data
        staff_list = Staff.objects.filter(tenant=tenant, is_active=True).select_related(
            "user", "department", "designation"
        )
...
class HRDashboardWidgetsView(BaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if widget_type == "StaffAttendance":
            today = timezone.now().date()
            StaffAttendance = StaffAttendance.objects.filter(tenant=tenant, date=today)

            data = {
                "present": StaffAttendance.filter(
            ...
            month_leaves = LeaveApplication.objects.filter(
                tenant=tenant, start_date__gte=current_month, status="APPROVED"
            ).count()

            data = {"pending": pending_leaves, "month_approved": month_leaves}
...
class StaffAttendanceView(BaseDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["present_count"] = (
            context["attendance_records"].filter(status="PRESENT").count()
        )
        context["absent_count"] = (
            context["attendance_records"].filter(status="ABSENT").count()
        )
        context["leave_count"] = (
            context["attendance_records"].filter(status="LEAVE").count()
        )
        context["late_count"] = (
            context["attendance_records"].filter(status="LATE").count()
        )
...
class StaffLeavesView(BaseDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        current_year = timezone.now().year
        context["leave_balances"] = LeaveBalance.objects.filter(
            staff=staff, year=current_year
        ).select_related("leave_type")

        # Leave summary
        context["total_leaves"] = context["leave_applications"].count()
        context["approved_leaves"] = (
            context["leave_applications"].filter(status="APPROVED").count()
        )
        context["pending_leaves"] = (
            context["leave_applications"].filter(status="PENDING").count()
        )

        # Current month leaves
        current_month = timezone.now().date().replace(day=1)
        context["current_month_leaves"] = context["leave_applications"].filter(
            start_date__gte=current_month, status="APPROVED"
        )
...
class StaffDocumentsView(BaseDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["verified_documents"] = (
            context["documents"].filter(is_verified=True).count()
        )
        context["expired_documents"] = (
            context["documents"].filter(expiry_date__lt=timezone.now().date()).count()
        )
...
class StaffSalaryView(BaseDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Get payroll records
        context["payroll_records"] = Payroll.objects.filter(staff=staff).order_by(
            "-salary_month"
        )
        ...
        if context["payroll_records"].exists():
            ...
            context["avg_monthly_salary"] = (
                total_salary / context["payroll_records"].count()
            )
            ...
            try:
                context["current_month_payroll"] = Payroll.objects.get(
                    staff=staff, salary_month=current_month
                )
...
class StaffPerformanceView(BaseDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["performance_reviews"] = (
            PerformanceReview.objects.filter(staff=staff)
        ...
        if context["performance_reviews"].exists():
            ...
            context["avg_rating"] = reviews.aggregate(avg=Avg("overall_rating"))["avg"]
        ...
        # Training statistics
        context["completed_trainings"] = (
            context["trainings"].filter(status="COMPLETED").count()
        )
        context["certified_trainings"] = (
            context["trainings"].filter(certificate_issued=True).count()
        )
...
class DesignationCreateView(BaseCreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f"Designation '{form.instance.title}' created successfully."
        )
...
class DesignationUpdateView(BaseUpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f"Designation '{form.instance.title}' updated successfully."
        )
...
class DesignationDeleteView(BaseDeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if designation.staff_members.filter(is_active=True).exists():
            ...
            return JsonResponse(
                {"success": False, "error": "Designation has active staff"}
            )
        ...
        messages.success(
            request, f"Designation '{designation.title}' has been deleted."
        )
...
class DepartmentDetailView(BaseDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["recent_leaves"] = (
            LeaveApplication.objects.filter(staff__department=department)
            .select_related("staff", "leave_type")
            .order_by("-applied_date")[:5]
        )
...
class AttendanceDailyView(BaseTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["present_count"] = len(
            [r for r in attendance_records if r.status in ["PRESENT", "LATE"]]
        )
        context["absent_count"] = len(
            [r for r in attendance_records if r.status == "ABSENT"]
        )
        context["leave_count"] = len(
            [r for r in attendance_records if r.status == "LEAVE"]
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        for staff in all_staff:
            ...
            if staff_records.exists():
                present_days = staff_records.filter(
                    status__in=["PRESENT", "LATE"]
                ).count()
        ...
        # Overall statistics
        if monthly_summary:
            context["avg_attendance"] = sum(
                item["attendance_percentage"] for item in monthly_summary
            ) / len(monthly_summary)
            context["best_attendance"] = max(
                monthly_summary, key=lambda x: x["attendance_percentage"]
            )
            context["worst_attendance"] = min(
                monthly_summary, key=lambda x: x["attendance_percentage"]
            )
        ...
        return JsonResponse(
            {
                ...
                "overall": {
                    ...
                    "leave": leave,
                    "present_percentage": (
                        (present / total_staff * 100) if total_staff > 0 else 0
                    ),
                },
...
class LeaveTypeCreateView(BaseCreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f"Leave type '{form.instance.name}' created successfully."
        )
...
class LeaveTypeUpdateView(BaseUpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f"Leave type '{form.instance.name}' updated successfully."
        )
...
class LeaveTypeDeleteView(BaseDeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if leave_type.applications.exists():
            ...
            return JsonResponse(
                {"success": False, "error": "Leave type has applications"}
            )
        ...
        messages.success(request, f"Leave type '{leave_type.name}' has been deleted.")
...
class LeaveApplicationUpdateView(BaseUpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f"Leave application updated successfully.")
...
class LeaveApplicationCancelView(BaseView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        messages.success(request, f"Leave application has been cancelled.")
...
class LeaveApplicationDeleteView(BaseDeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        messages.success(request, f"Leave application has been deleted.")
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.object_list:
            ...
            context["summary"] = {
                ...
                "utilization_rate": (
                    (total_used / total_entitled * 100) if total_entitled > 0 else 0
                ),
            }
...
class PayrollProcessView(BaseTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        draft_payroll = Payroll.objects.filter(
            tenant=tenant, salary_month=month_date, status="DRAFT"
        ).select_related("staff", "staff__user", "staff__department")

        context["draft_payroll"] = draft_payroll
        context["selected_month"] = month_date
        context["total_amount"] = (
            sum(p.net_salary for p in draft_payroll) if draft_payroll else 0
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not payroll_ids:
            return JsonResponse(
                {"success": False, "error": "No payroll records selected"}
            )
...
class PayrollDeleteView(BaseDeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if payroll.status != "DRAFT":
            messages.error(
                request, f"Cannot delete payroll record with status: {payroll.status}"
            )
            return JsonResponse(
                {
                    "success": False,
        ...
        messages.success(
            request, f"Payroll record for {payroll.staff.full_name} has been deleted."
        )
...
class PayrollApproveView(BaseView):
    ...
    def post(self, request, *args, **kwargs):
        payroll = get_object_or_404(
            Payroll.objects.filter(tenant=get_current_tenant()), pk=kwargs["pk"]
        )
        ...
        messages.success(
            request, f"Payroll for {payroll.staff.full_name} has been approved."
        )
        ...
        return JsonResponse(
            {"success": True, "message": "Payroll approved successfully."}
        )
...
class PayrollMonthlyReportView(BaseTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        payroll_records = Payroll.objects.filter(
            tenant=tenant, salary_month=month_date, status="PAID"
        ...
        for payroll in payroll_records:
            ...
            if dept_name not in departments:
                departments[dept_name] = {"records": [], "total_salary": 0, "count": 0}
...
class PayrollAnnualReportView(BaseTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        payroll_records = Payroll.objects.filter(
            tenant=tenant, salary_month__year=year, status="PAID"
        ).select_related("staff", "staff__department")
        ...
        for month in range(1, 13):
            ...
            if month_payroll.exists():
                ...
                monthly_data[month] = {
                    ...
                    "avg_salary": (
                        total_salary / month_payroll.count()
                        if month_payroll.count() > 0
                        else 0
                    ),
                }
        ...
        context["total_salary"] = sum(float(p.net_salary) for p in payroll_records)
        context["avg_monthly_salary"] = (
            context["total_salary"] / 12 if payroll_records.count() > 0 else 0
        )
                ...
                dept_data.append(
                    {
                        ...
                        "total_salary": total_salary,
                        "percentage": (
                            (total_salary / context["total_salary"] * 100)
                            if context["total_salary"] > 0
                            else 0
                        ),
                    }
                )
                    ...
                    department_data.append(
                        {
                            ...
                            "total_salary": total_salary,
                            "avg_salary": (
                                total_salary / payroll_records.count()
                ...
                staff_data.append(
                    {
                        ...
                        "total_salary": total_salary,
                        "avg_salary": (
                            total_salary / staff_payroll.count()
                            if staff_payroll.count() > 0
                            else 0
                        ),
                    }
                )
...
class PayrollSummaryView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        payroll_records = Payroll.objects.filter(
            tenant=tenant, salary_month=month_date
...
class RecruitmentListView(BaseListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.object_list:
            ...
            context["open_positions"] = sum(
                r.no_of_openings for r in self.object_list if r.is_open
            )
...
class RecruitmentCreateView(BaseCreateView):
    ...
    def form_valid(self, form):
        ...
        audit_log(
            ...
            resource_id=str(self.object.id),
            details={
                "position": self.object.position_title,
                "openings": self.object.no_of_openings,
            },
            severity="INFO",
        )
...
class RecruitmentDetailView(BaseDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["applications"] = recruitment.applications.all().order_by(
            "-applied_date"
        )
...
class RecruitmentDeleteView(BaseDeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if recruitment.applications.exists():
            ...
            return JsonResponse(
                {"success": False, "error": "Recruitment has applications"}
            )
...
class RecruitmentCloseView(BaseView):
    ...
    def post(self, request, *args, **kwargs):
        recruitment = get_object_or_404(
            Recruitment.objects.filter(tenant=get_current_tenant()), pk=kwargs["pk"]
        )
...
class JobApplicationListView(BaseListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["recruitments"] = Recruitment.objects.filter(
...
class JobApplicationCreateView(BaseCreateView):
    ...
    def form_valid(self, form):
        ...
        audit_log(
            ...
            resource_id=str(self.object.id),
            details={
                "applicant": self.object.applicant_name,
                "position": self.object.recruitment.position_title,
            },
            severity="INFO",
        )
...
class JobApplicationShortlistView(BaseView):
    ...
    def post(self, request, *args, **kwargs):
        application = get_object_or_404(
            JobApplication.objects.filter(tenant=get_current_tenant()), pk=kwargs["pk"]
        )

        if application.status != "APPLIED":
            messages.error(
                request,
                f"Cannot shortlist application with status: {application.get_status_display()}",
            )
...
class JobApplicationInterviewView(BaseView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if application.status != "SHORTLISTED":
            messages.error(
                request,
                f"Cannot schedule interview for application with status: {application.get_status_display()}",
            )
            return redirect("hr:jobapplication_detail", pk=application.pk)
...
class JobApplicationOfferView(BaseView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return JsonResponse(
            {"success": True, "message": "Job offer made successfully."}
        )
...
class TrainingProgramDetailView(BaseDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["available_slots"] = training.remaining_seats
...
class TrainingProgramDeleteView(BaseDeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if training.participants.exists():
            ...
            return JsonResponse(
                {"success": False, "error": "Training program has participants"}
            )
        ...
        messages.success(
            request, f"Training program '{training.title}' has been deleted."
        )
...
class TrainingRegisterView(BaseView):
    ...
    def post(self, request, *args, **kwargs):
        training = get_object_or_404(
            TrainingProgram.objects.filter(tenant=get_current_tenant()), pk=kwargs["pk"]
        )
        ...
        audit_log(
            ...
            resource_type="Training Program",
            resource_id=str(training.id),
            details={"training": training.title, "staff": staff.full_name},
            severity="INFO",
        )
...
class TrainingParticipationListView(BaseListView):
    ...
    def get_queryset(self):
        ...
        if certificate_issued and certificate_issued != "all":
            queryset = queryset.filter(
                certificate_issued=(certificate_issued == "true")
            )
...
class TrainingParticipationCreateView(BaseCreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f"Training participation record created successfully."
        )
...
class TrainingParticipationUpdateView(BaseUpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f"Training participation record updated successfully."
        )
...
class TrainingParticipationDeleteView(BaseDeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        messages.success(request, f"Training participation record has been deleted.")
...
class PerformanceReviewListView(BaseListView):
    ...
    def get_queryset(self):
        ...
        if not (
            user.has_perm("hr.view_all_performance")
            or user.role in ["admin", "hr_manager"]
        ):
            queryset = queryset.filter(Q(staff__user=user) | Q(reviewed_by=user))
...
class PerformanceReviewCreateView(BaseCreateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "hr:performancereview_detail", kwargs={"pk": self.object.pk}
        )
    ...
    def form_valid(self, form):
        ...
        audit_log(
            ...
            details={
                "staff": self.object.staff.full_name,
                "rating": self.object.overall_rating,
            },
            severity="INFO",
        )
...
class PerformanceReviewDetailView(BaseDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["can_acknowledge"] = (
            self.request.user == review.staff.user and not review.employee_acknowledged
        )

        # Check if user can approve
        context["can_approve"] = (
            self.request.user.has_perm("hr.approve_performancereview")
            and review.approved_by is None
        )
...
class PerformanceAcknowledgeView(BaseView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if review.employee_acknowledged:
            return JsonResponse(
                {"success": False, "error": "Performance review already acknowledged."}
            )
        ...
        audit_log(
            ...
            resource_type="Performance Review",
            resource_id=str(review.id),
            details={"staff": review.staff.full_name, "rating": review.overall_rating},
            severity="INFO",
        )
...
class EmploymentHistoryCreateView(BaseCreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f"Employment history record created successfully."
        )
...
class PromotionListView(BaseListView):
    ...
    def get_queryset(self):
        ...
        queryset = (
            ...
            .filter(tenant=tenant)
            .select_related(
                "staff", "previous_designation", "new_designation", "staff__user"
            )
        )
...
class StaffDocumentDeleteView(BaseDeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        messages.success(request, f"Document '{document.file_name}' has been deleted.")
    ...
    def form_valid(self, form):
        ...
        if form.instance.is_current:
            StaffAddress.objects.filter(
                staff=form.instance.staff, address_type=form.instance.address_type
...
class StaffAddressDeleteView(BaseDeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        other_addresses = StaffAddress.objects.filter(
            staff=address.staff, address_type=address.address_type
        ...
        messages.success(
            request, f"Address for {address.staff.full_name} has been deleted."
        )
...
class StaffReportView(BaseTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Age distribution
        age_groups = {
            "Under 30": staff_queryset.filter(
                date_of_birth__gte=timezone.now() - timezone.timedelta(days=30 * 365)
            ).count(),
            ...
            "Over 60": staff_queryset.filter(
                date_of_birth__lt=timezone.now() - timezone.timedelta(days=60 * 365)
            ).count(),
        }
...
class AttendanceReportView(BaseTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        attendance_map = {
            (r["staff_id"], r["date"]): r["status"] for r in attendance_records
        }
...
class LeaveReportView(BaseTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        leave_apps = LeaveApplication.objects.filter(
            tenant=tenant, start_date__year=year, status="APPROVED"
    ...
    def get_context_data(self, **kwargs):
        ...
        for year in range(start_year, end_year + 1):
            ...
            joined = Staff.objects.filter(
                tenant=tenant, joining_date__year=year, is_active=True
            ).count()
        ...
        for dept in departments:
            ...
            if dept_staff.exists():
                joined = dept_staff.filter(
                    joining_date__year=end_year, is_active=True
                ).count()
...
class DemographicReportView(BaseTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Age distribution
        age_groups = {
            "Under 25": staff_queryset.filter(
                date_of_birth__gte=timezone.now() - timezone.timedelta(days=25 * 365)
            ).count(),
            ...
            "Over 65": staff_queryset.filter(
                date_of_birth__lt=timezone.now() - timezone.timedelta(days=65 * 365)
            ).count(),
        }
                ...
                desig_salary_stats.append(
                    {
                        ...
                        "avg_salary": (
                            sum(desig_salaries) / len(desig_salaries)
                            if desig_salaries
                            else 0
                        ),
                ...
                emp_type_stats.append(
                    {
                        ...
                        "avg_salary": (
                            sum(emp_salaries) / len(emp_salaries) if emp_salaries else 0
                        ),
                ...
                gender_stats.append(
                    {
                        ...
                        "avg_salary": (
                            sum(gender_salaries) / len(gender_salaries)
                            if gender_salaries
                            else 0
                        ),
...
class HRSettingsView(BaseTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["leave_type_count"] = LeaveType.objects.filter(
            tenant=tenant, is_active=True
        ).count()
        context["training_program_count"] = TrainingProgram.objects.filter(
            tenant=tenant
        ).count()
...
class ReportGenerateView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if report_type not in valid_reports:
            return JsonResponse(
                {"success": False, "error": f"Invalid report type: {report_type}"}
            )
        ...
        if format_type == "pdf":
            ...
            file_name = (
                f"{report_type}_report_{timezone.now().strftime('%Y%m%d_%H%M%S')}.pdf"
            )
            content_type = "application/pdf"
        elif format_type == "excel":
            file_content = self.generate_excel_report(report_type, report_data)
            file_name = (
                f"{report_type}_report_{timezone.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
            )
            content_type = (
            ...
            return JsonResponse(
                {"success": False, "error": f"Invalid format type: {format_type}"}
            )
    ...
    def generate_report_data(self, report_type, parameters):
        ...
        if report_type == "staff":
            ...
            return {
                ...
                "departments": Department.objects.filter(tenant=tenant),
                "staff_by_dept": staff_queryset.values("department__name").annotate(
                    count=Count("id")
                ),
                "staff_by_designation": staff_queryset.values(
                    "designation__title"
            ...
            staff_attendance_qs = (
                ...
                .select_related("staff", "staff__user", "staff__department")
                .order_by("date", "staff__user__first_name")
            )
            ...
            return {
                ...
                "attendance_by_status": staff_attendance_qs.values("status").annotate(
                    count=Count("id")
                ),
                "attendance_by_dept": staff_attendance_qs.values(
                    "staff__department__name"
                ).annotate(count=Count("id")),
                "records": list(
                    records
                ),  # Convert queryset to list for passing to export functions
            }
            ...
            stats = {
                ...
                "median_salary": (
                    sorted(salaries)[len(salaries) // 2] if salaries else 0
                ),
                "staff_count": len(salaries),
            }
            ...
            for d in departments:
                d_staff = [
                    ...
                    if s.department_id == d.id and s.basic_salary
                ]
                if d_staff:
            ...
            for desig in designations:
                desig_staff = [
                    s
                    for s in staff_queryset
                    if s.designation_id == desig.id and s.basic_salary
                ]
                if desig_staff:
                    desig_salaries = [s.basic_salary for s in desig_staff]
                    desig_salary_stats.append(
        ...
        if report_type == "StaffAttendance":
            ...
            for record in data.get("records", []):
                ...
                ws.cell(
                    row=row_num,
                    column=3,
                    value=f"{record['staff__user__first_name']} {record['staff__user__last_name']}",
                )
                ...
                ws.cell(
                    row=row_num,
                    column=6,
                    value=str(record["check_in"]) if record["check_in"] else "",
                )
                ws.cell(
                    row=row_num,
                    column=7,
                    value=str(record["check_out"]) if record["check_out"] else "",
                )
                row_num += 1
            ...
            for staff in data.get("staff_list", []):
                ...
                ws.cell(
                    row=row_num,
                    column=2,
                    value=f"{staff['user__first_name']} {staff['user__last_name']}",
                )
...
class HolidayCreateView(BaseCreateView):
    ...
    def form_valid(self, form):
        ...
        audit_log(
            self.request.user,
            "Created Holiday",
            f"Created holiday {form.instance.name}",
        )
        return super().form_valid(form)
...
class HolidayUpdateView(BaseUpdateView):
    ...
    def form_valid(self, form):
        audit_log(
            self.request.user,
            "Updated Holiday",
            f"Updated holiday {form.instance.name}",
        )
        return super().form_valid(form)
...
class HolidayDeleteView(BaseDeleteView):
    ...
    def form_valid(self, form):
        audit_log(
            self.request.user, "Deleted Holiday", f"Deleted holiday {self.object.name}"
        )
        return super().form_valid(form)
...
class WorkScheduleCreateView(BaseCreateView):
    ...
    def form_valid(self, form):
        ...
        audit_log(
            ...
            f"Created work schedule {form.instance.name}",
        )
        return super().form_valid(form)
...
class WorkScheduleUpdateView(BaseUpdateView):
    ...
    def form_valid(self, form):
        audit_log(
            ...
            f"Updated work schedule {form.instance.name}",
        )
        return super().form_valid(form)
...
class WorkScheduleDeleteView(BaseDeleteView):
    ...
    def form_valid(self, form):
        audit_log(
            self.request.user,
            "Deleted Work Schedule",
            f"Deleted work schedule {self.object.name}",
        )
        return super().form_valid(form)
...
class DisciplinaryActionListView(BaseListView):
    model = DisciplinaryAction
    template_name = "hr/disciplinary/action_list.html"
    context_object_name = "actions"
    permission_required = "hr.view_disciplinaryaction"
    paginate_by = 20

    def get_queryset(self):
        queryset = super().get_queryset().select_related('staff', 'staff__user')
        search = self.request.GET.get("search", "").strip()
        if search:
            queryset = queryset.filter(
                ...
                | Q(description__icontains=search)
                | Q(violation__icontains=search)
            )
        return queryset.order_by("-action_date")

class DisciplinaryActionCreateView(BaseCreateView):
    model = DisciplinaryAction
    form_class = DisciplinaryActionForm
    template_name = "hr/disciplinary/action_form.html"
    permission_required = "hr.add_disciplinaryaction"
    roles_required = ["admin", "hr_manager"]

    def get_success_url(self):
        return reverse_lazy("hr:disciplinary_list")

    def form_valid(self, form):
        form.instance.issued_by = self.request.user
        response = super().form_valid(form)
        messages.success(self.request, _("Disciplinary action recorded successfully."))
        return response

class DisciplinaryActionUpdateView(BaseUpdateView):
    model = DisciplinaryAction
    form_class = DisciplinaryActionForm
    template_name = "hr/disciplinary/action_form.html"
    permission_required = "hr.change_disciplinaryaction"
    roles_required = ["admin", "hr_manager"]

    def get_success_url(self):
        return reverse_lazy("hr:disciplinary_list")

class DisciplinaryActionDeleteView(BaseDeleteView):
    model = DisciplinaryAction
    template_name = "hr/common/confirm_delete.html"
    permission_required = "hr.delete_disciplinaryaction"
    roles_required = ["admin", "hr_manager"]
...
class AssetListView(BaseListView):
    model = Asset
    template_name = "hr/asset/asset_list.html"
    context_object_name = "assets"
    permission_required = "hr.view_asset"
    paginate_by = 20
...
class AssetCreateView(BaseCreateView):
    ...
    template_name = "hr/asset/asset_form.html"
    permission_required = "hr.add_asset"
    roles_required = ["admin", "it_staff", "hr_manager"]

    def get_success_url(self):
        return reverse_lazy("hr:asset_list")

    def form_valid(self, form):
        messages.success(self.request, _("Asset created successfully."))
        return super().form_valid(form)
...
class AssetUpdateView(BaseUpdateView):
    ...
    form_class = AssetForm
    template_name = "hr/asset/asset_form.html"
    permission_required = "hr.change_asset"
    roles_required = ["admin", "it_staff", "hr_manager"]
...
class AssetDeleteView(BaseDeleteView):
    model = Asset
    template_name = "hr/common/confirm_delete.html"
    permission_required = "hr.delete_asset"
    roles_required = ["admin", "it_staff", "hr_manager"]
...
class AssetAssignmentListView(BaseListView):
    model = AssetAssignment
    template_name = "hr/asset/assignment_list.html"
    context_object_name = "assignments"
    permission_required = "hr.view_assetassignment"
    paginate_by = 20

    def get_queryset(self):
        queryset = super().get_queryset().select_related("staff", "asset")
        search = self.request.GET.get("search", "").strip()
        if search:
            queryset = queryset.filter(
                Q(staff__first_name__icontains=search)
...
class AssetAssignmentCreateView(BaseCreateView):
    model = AssetAssignment
    form_class = AssetAssignmentForm
    template_name = "hr/asset/assignment_form.html"
    permission_required = "hr.add_assetassignment"
    roles_required = ["admin", "it_staff", "hr_manager"]

    def get_success_url(self):
        return reverse_lazy("hr:asset_assignment_list")

    def form_valid(self, form):
        form.instance.assigned_by = self.request.user
        response = super().form_valid(form)
        messages.success(self.request, _("Asset assigned successfully."))
        return response

class AssetAssignmentUpdateView(BaseUpdateView):
    model = AssetAssignment
    form_class = AssetAssignmentForm
    template_name = "hr/asset/assignment_form.html"
    permission_required = "hr.change_assetassignment"
    roles_required = ["admin", "it_staff", "hr_manager"]

    def get_success_url(self):
        return reverse_lazy("hr:asset_assignment_list")

class AssetAssignmentDeleteView(BaseDeleteView):
    model = AssetAssignment
    template_name = "hr/common/confirm_delete.html"
    permission_required = "hr.delete_assetassignment"
    roles_required = ["admin", "it_staff", "hr_manager"]
